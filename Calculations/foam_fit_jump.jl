...
for k=1:4
	...
	w=ones(length(muf_exp[k,:]))
	w[end-3:end-1]=10
	@setNLObjective(m, Min, sum{w[i]*(f_foam[k,i]-fmmob*(0.5+atan(epdry*(sw_exp[k,i]-fmdry))/pi))^2, i=1:8})
	...
	three_par[:,k]=x1[:]
	# and visualize
	sw_val = collect(linspace(0,1.0, 100))
	fg_opt = fg(sw_val, x1)
	...
	setValue(fmdry, x1[3])
	# define the objective function
	@setNLObjective(m, Min, sum{w[i]*(f_foam[k,i]-fmmob*(0.5+atan(epdry*(sw_exp[k,i]-fmdry))/pi)
	...
	figure(3)
	plot(fg2(sw_plot, x2[1:3], x2[4:5], muf_opt), mu_foam2(sw_plot, x2[1:3], x2[4:5], muf_opt),
		fg_exp', muf_exp[k,:]', "o")
	axis([0,1,0,0.25])
end
